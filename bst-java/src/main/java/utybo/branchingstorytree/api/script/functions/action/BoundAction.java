...
public class BoundAction implements ScriptAction
{
    ...
    public void exec(final String head, final String desc, final int line, final BranchingStory story, final BSTClient client) throws BSTException
    {
        ...
        if(bits.length != 3)
        {
            throw new BSTException(line, "Invalid syntax : bound:tocheck,min,max", story.getTag("__sourcename"));
        }
        if(registry.typeOf(bits[0]) != Integer.class && registry.typeOf(bits[0]) != null)
        {
            throw new BSTException(line, "Unknown variable : " + bits[0], story.getTag("__sourcename"));
        }
        ...
        if(max < min)
        {
            throw new BSTException(line, "min < max", story.getTag("__sourcename"));
        }
        if(toCheck < min)
        {
            toCheck = min;
